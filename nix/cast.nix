# The first argument requires all the necessary tools
# This is to be used with callPackage
{ autorecorder
, lib
, stdenv
, writeText
...
# The second argument lets the user pass in custom packages to be used in the cast themselves
{ pkgs
}:
with lib;
let
        ...
        builtins.fromJSON (
          builtins.readFile (
            stdenv.mkDerivation {
              name = "fromYAML";
              phases = [ "buildPhase" ];
              buildPhase = "${yq}/bin/yq 'del(.input)' ${src} > $out";
            }
          )
        );
    in
          ...
          makeWorkingDirScript = dir:
            ...
            ''
              ...
              ''}
                ...
                ${rsync}/bin/rsync --no-p --no-g --chmod=ugo=rwX -r ${dirToInclude}/ ${dirs.includeDirDestination}

                ${optionalString debug "${tree}/bin/tree"}
